...
function actualizarTotal() {
    const totalCalculado = productosEnCarrito.reduce((acc, producto) => {
        // Convierte precio_unidad y cantidad a números antes de la operación
        const subtotal = parseFloat(producto.precio_unidad) * parseInt(producto.cantidad);
        return acc + subtotal;
    }, 0);

    // Actualiza el contenido del elemento HTML
    contenedorTotal.innerText = `$${totalCalculado.toFixed(2)}`; // .toFixed(2) para mostrar dos decimales
}
